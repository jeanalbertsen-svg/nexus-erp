...
const LineSchema = new mongoose.Schema(
  {
    account: { type: String, required: true, index: true }, // "1000"
...
const JournalEntrySchema = new mongoose.Schema(
  {
    jeNumber: { type: String, required: true, unique: true, index: true }, // e.g., "JE-20250201-0003"
    ...
    status: {
      type: String,
      enum: ["draft", "approved", "posted", "reversed"],
      default: "draft",
      index: true,
    },

    currency: {
      code: { type: String, default: "DKK" },
    ...
    // optional grouping to “pin” subsequent entries under a common header
...
// validation: at least 2 non-zero lines and balanced
JournalEntrySchema.pre("validate", function (next) {
  ...
  if (nonZero.length < 2) {
    return next(new Error("Entry must contain at least 2 non-zero lines."));
  }
