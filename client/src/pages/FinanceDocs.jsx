...
import {
  ...
  IconButton, Box, ToggleButton, ToggleButtonGroup, Dialog, DialogTitle,
...
const BRAND = {
  name: "ACTA Ventures",
  address: "Økonomivej 1, 2100 København • CVR 12345678",
...
/* ---------- Daily report HTML ---------- */
function renderDailyReportHTML(date, sales, purch) {
  ...
  const purchRows = purch.length
    ? purch.map(b=>`<tr><td>${b.docNo}</td><td>${b.party||"-"}</td><td class="num">${fmtKr(b.total)}</td></tr>`).join("")
    : `<tr><td colspan="3" class="small muted">None.</td></tr>`;
  ...
  <table class="report">
    ...
    <tfoot>
      <tr><td colspan="2">Purchases total</td><td class="num">${fmtKr(sum(purch))}</td></tr>
    </tfoot>
  </table>

  <h3>Net for the day</h3>
  <p><strong>${fmtKr(sum(sales) - sum(purch))}</strong></p>
  `;
}
...
/* -------------------------------------------------------
   Description templates (dropdown options)
------------------------------------------------------- */
              ...
              <TableRow key={idx}>
                ...
                <TableCell>
                  <Autocomplete
                    ...
                    renderInput={(p)=><TextField {...p} placeholder={accountType==="REVENUE"?"4000 • Sales Revenue":"5000 • COGS / Expense"} />}
                  />
                </TableCell>
...
const glFromPurchaseInvoice = ({ bill, apAccount="2000", inputTaxAssetAccount="1300" }) => {
  ...
  const ref = bill.referenceNo || `BILL-${day}-${String(bill.autoSeq).padStart(4,"0")}`;
...
/* ---------- Renderers: Sales/Purchase layout ---------- */
function renderInvoiceHTML(doc) {
  ...
  return `
    <div class="hdr">
      ...
      <div class="right">
        <h2>${isSales ? "Invoice" : "Purchase Invoice"}</h2>
    ...
    <div class="grid" style="margin-bottom:10px">
      ...
      <div>
        <div class="small muted">${partyLabel}</div>
        <div>${doc.party || "-"}</div>
      </div>
    </div>
    ...
    <table class="report inv">
      ...
      <tfoot>
        ...
        <tr><td colspan="5" class="right">${isSales ? "Sales tax / VAT" : "Input tax / VAT"}</td><td class="num">${fmt(tax)}</td></tr>
  ...
  return (
    <Paper variant="outlined" sx={{ p:2, borderRadius:2, borderColor: paperBor }}>
      ...
      <InvoiceLines
        ...
        mergedOptions={mergedOptions}
        accountType="EXPENSE"
        descriptionOptions={DESCRIPTION_TEMPLATES.PURCHASE}
      />
...
/* -------------------------------------------------------
   DAILY REPORT component (re-usable)
------------------------------------------------------- */
function DailyReport({ docs }) {
  ...
  const purch = dayDocs.filter(d=>d.type==="Purchase");
  const html = renderDailyReportHTML(date, sales, purch);
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, borderColor: paperBor }}>
      <Stack direction={{ xs: "column", sm: "row" }} spacing={1} alignItems="center" sx={{ mb: 1 }}>
        ...
        <Button startIcon={<PictureAsPdfIcon />} color="primary" variant="outlined" onClick={() => printHtml(`Daily Report ${date}`, html)}>Save as PDF</Button>
      </Stack>
      <Divider sx={{ mb: 0.5 }} />
      <div
...
/* -------------------------------------------------------
   Helpers: persisted docs (separate stores)
------------------------------------------------------- */
...
/* -------------------------------------------------------
   SALES PAGE (own docs + click-to-open form)
------------------------------------------------------- */
function SalesInvoicesPage({ accounts = [], onAddGlMany, onAddMany }) {
  ...
  return (
    <Stack spacing={2}>
      ...
      {/* Daily report for SALES page only (still useful for export) */}
          ...
          <div
            dangerouslySetInnerHTML={{
              __html:
                `<style>
                  :root{ --brand:${BRAND.primary}; --th-bg:${BRAND.primaryLight}; --th-fg:#0b2b55; --line:${BRAND.primaryBorder}; --muted:#56657a; --fg:#0b1650; --bg:#ffffff; }
                  .preview-surface{!important; !important; padding:0; border-radius:8px; }
                  table.report.inv{ width:100%; border-collapse:collapse; table-layout:fixed; }
                  table.report.inv colgroup col:nth-child(1){ width:38%; }
                  table.report.inv colgroup col:nth-child(2){ width:10%; }
                  table.report.inv colgroup col:nth-child(3){ width:18%; }
...
/* -------------------------------------------------------
   PURCHASES PAGE (own docs + click-to-open form)
------------------------------------------------------- */
function PurchaseInvoicesPage({ accounts = [], onAddGlMany, onAddMany }) {
  const theme = useTheme();
  const isDark = theme.palette.mode === "dark";
  const headBg  = alpha(theme.palette.primary.main, isDark ? 0.22 : 0.12);
  const headBor = alpha(theme.palette.primary.main, isDark ? 0.50 : 0.30);
  ...
  return (
    <Stack spacing={2}>
      ...
      {/* Daily report for PURCHASE page */}
      <DailyReport docs={docs} />
          ...
          <div
            dangerouslySetInnerHTML={{
              __html:
                `<style>
                  :root{ --brand:${BRAND.primary}; --th-bg:${BRAND.primaryLight}; --th-fg:#0b2b55; --line:${BRAND.primaryBorder}; --muted:#56657a; --fg:#0b1650; --bg:#ffffff; }
                  .preview-surface{!important; color:var(--fg) !important; padding:0; border-radius:8px; }
                  table.report.inv{ width:100%; border-collapse:collapse; table-layout:fixed; }
                  table.report.inv colgroup col:nth-child(1){ width:38%; }
                  table.report.inv colgroup col:nth-child(2){ width:10%; }
                  table.report.inv colgroup col:nth-child(3){ width:18%; }
...
/* -------------------------------------------------------
   MAIN WRAPPER — choose page (can be replaced by real routing)
------------------------------------------------------- */
export default function InvoicesApp({ accounts = [], onAddGlMany, onAddMany }) {
  const [page, setPage] = useState("sales");
  return (
    <Stack spacing={2}>
      <ToggleButtonGroup size="small" exclusive value={page} onChange={(_, v) => v && setPage(v)}>
        <ToggleButton value="sales" color="primary">Sales</ToggleButton>
        <ToggleButton value="purchase" color="primary">Purchases</ToggleButton>
      </ToggleButtonGroup>
      {page === "sales" ? (
        <SalesInvoicesPage accounts={accounts} onAddGlMany={onAddGlMany} onAddMany={onAddMany} />
      ) : (
        <PurchaseInvoicesPage accounts={accounts} onAddGlMany={onAddGlMany} onAddMany={onAddMany} />
      )}
    </Stack>
  );
}

/* Optionally export the pages for use with a router (e.g., React Router) */
export { SalesInvoicesPage, PurchaseInvoicesPage };
